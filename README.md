...
6. **The honest ledger**: loads avoided as a lower bound, `≥N (estimate)` always labeled, chars/4 stated as the approximation. No per-rule attribution, ever: there is no observable counterfactual, so that number would be theater. The rule underneath is the one every honest audit follows: never state as fact what you cannot verify. I bank the coin I can assay and label the rest an estimate. And one coin I never book as saved: output squeezed until the reader has to read it twice. Brevity that costs comprehension is debt, not savings; trim by dropping what the reader does not need, never by mangling what they do.
